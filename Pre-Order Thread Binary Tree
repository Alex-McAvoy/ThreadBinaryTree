...
template<class T>
void ThreadTree<T>::preThread(Node<T> *bt,Node<T> *pre){//前序线索化
    if(root!=NULL){
        ...
        pre=bt;//保持pre指向bt的前驱
        preThread(bt->lchild,pre);//递归左子树线索化
        preThread(bt->rchild,pre);//递归右子树线索化
    }
}
template<class T>
...
template<class T>
void ThreadTree<T>::preOrder(){//遍历
    if(root==NULL)
        return;
    else{
        Node<T> *p;
        p=root;
        while(p->ltag==link)//寻找前序遍历序列的第一个结点
            p=p->lchild;
        cout<<p->data;
