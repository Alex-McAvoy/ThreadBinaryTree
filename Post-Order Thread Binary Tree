...
template<class T>
void ThreadTree<T>::postThread(Node<T> *bt,Node<T> *pre){//后序线索化
    if(root!=NULL){
        postThread(bt->lchild,pre);//递归左子树线索化
        postThread(bt->rchild,pre);//递归右子树线索化
...
template<class T>
void ThreadTree<T>::postOrder(){//遍历
    if(root==NULL)
        return;
    else{
        Node<T> *p;
        p=root;
        while(p->ltag==link)//寻找后序遍历序列的第一个结点
            p=p->lchild;
        cout<<p->data;
